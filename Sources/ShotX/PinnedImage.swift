...
final class PinnedImageController {
    ...
    private func close(panel: NSPanel) {
        // Pinned captures hold a full-resolution NSImage; `orderOut` alone would
        // keep every unpinned one alive for the rest of the session.
...
private struct PinnedImageView: View {
    ...
    var body: some View {
        ZStack(alignment: .topTrailing) {
            ...
            if hovering {
                HStack(spacing: 6) {
                    PinActionButton(icon: "pencil.tip", tooltip: "编辑", action: onEdit)
                    PinActionButton(icon: "doc.on.doc", tooltip: "复制", action: onCopy)
                    PinActionButton(icon: "square.and.arrow.down", tooltip: "保存", action: onSave)
                    PinActionButton(icon: "xmark", tooltip: "取消贴图", tint: .red, action: onClose)
                }
                .padding(8)
