...
final class WindowCaptureController {
    ...
    private func capture(window w: CapturableWindow) {
        tearDownOverlay()
        // No settle delay needed: `.optionIncludingWindow` composites that one
        // window's backing store, so our picker overlay can't appear in it.
...
/// A borderless `NSWindow` refuses key status by default, which would leave
/// `WindowSelectionView`'s Escape handler dead — the picker would only ever see
/// mouse events, and there'd be no way to back out without clicking.
