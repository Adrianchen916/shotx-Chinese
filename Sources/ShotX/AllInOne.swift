...
private struct AllInOneView: View {
    ...
    private var options: [Option] {
        [
            Option(action: .area,       icon: "viewfinder",       label: "区域",      enabled: true),
            Option(action: .fullscreen, icon: "display",          label: "全屏",      enabled: true),
            Option(action: .window,     icon: "macwindow",        label: "窗口",      enabled: true),
            Option(action: .previous,   icon: "arrow.clockwise",  label: "上一区域",  enabled: LastCaptureStore.hasPrevious),
            Option(action: .timer,      icon: "timer",            label: "定时",      enabled: true),
            Option(action: .record,     icon: "video.fill",       label: "录屏",      enabled: true)
        ]
    }
