...
struct RecordingOptionsView: View {
    ...
    private var settingsStrip: some View {
        HStack(spacing: 10) {
            ...
            Spacer(minLength: 6)
            HStack(spacing: 2) {
                CompactToggle(icon: "mic", binding: $captureMicrophone, enabled: true, tooltip: "麦克风")
                CompactToggle(icon: "speaker.wave.2", binding: $captureSystemAudio, enabled: true, tooltip: "系统声音")
                CompactToggle(icon: "cursorarrow", binding: $showCursor, enabled: true, tooltip: "显示光标")
                CompactToggle(icon: "cursorarrow.click", binding: $highlightClicks, enabled: true, tooltip: "点击高亮")
                CompactToggle(icon: "camera", binding: $recordWebcam, enabled: true, tooltip: "摄像头画中画（左下角）")
                CompactToggle(icon: "keyboard", binding: .constant(false), enabled: false, tooltip: "按键显示（即将推出）")
            }
        }
